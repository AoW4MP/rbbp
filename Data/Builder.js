...
function showUnit(unitID, subcultureCheck, resID, divOrigin) {
    ...
    for (let x = 0; x < unitEN.primary_passives.length; x++) {
        ...
        if (unitEN.primary_passives[x].slug.indexOf("high_maintenance") != -1) {
            if (unitEN.upkeep.indexOf("influence") != -1) {
                tier.innerHTML =
                    "Tier " + romanize(unitEN.tier) + ": " + ReduceUpkeepPercentage(unitEN.upkeep, 1.20) + "*";
            } else {
                tier.innerHTML =
                    "Tier " + romanize(unitEN.tier) + ": " + ReduceUpkeepPercentage(unitEN.upkeep, 1.20) + ">*";
            }
            ...
            if (summonInfo.length > 0) {
                if (unitEN.upkeep.indexOf("influence") != -1) {
                    tier.innerHTML += "<br> Summoned: " + getSummonedUpkeep(unitEN.tier, 1.20) + "*";
                } else {
                    tier.innerHTML += "<br> Summoned: " + getSummonedUpkeep(unitEN.tier, 1.20) + ">*";
                }
            }
...
function backtrackUnitOrigins(unitData, name, holder) {
    ...
    for (let x = 0; x < spells.length; x++) {
        ...
        const imgSrc = `/rbbp/Icons/SpellIcons/${spells[x].icon}.png`;
        const imgFallbackSrc = `/rbbp/Icons/Text/mp.png`;
    ...
    if (siege != "") {
        ...
        const imgSrc = `/rbbp/Icons/SiegeProjectIcons/${siege.icon}.png`;
        const imgFallbackSrc = `/rbbp/Icons/Text/mp.png`;
    ...
    if (governance != "") {
        ...
        const imgSrc = `/rbbp/Icons/GovernanceIcons/${governance.icon}.png`;
        const imgFallbackSrc = `/rbbp/Icons/Text/mp.png`;
        const link = `/rbbp/HTML/Spells.html?governance=${governance.id}`;
        createUnitTypeIcon(holderOrigin, imgSrc, imgFallbackSrc, link, tooltipText);
    }
